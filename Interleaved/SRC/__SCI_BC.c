...
void scib_putc(char d)
{
	...
	SCIB_TX_START;	// 2011_03_07 dbsgln
}
		...
		else//crc_L
		{
			...
			if((RxBuf[7] == CRC.Byte.b1) && (RxBuf[8] == CRC.Byte.b0))
			{
				...
				if(RxType == SEND)
				{
					...
					// (110107 by HHH)
					Rx_index= RxAddr;
					Read_Data_Registers(Rx_index);	// 직접 RxAddr 및 RxData 를 읽는다
...
else
{
		...
		else if(SciC_TxFlag)
		{
			...
			// (110107 by HHH)
			//Flag.Monitoring.bit.EEPROM_WRITE_ENABLE_Tx= 1;  // RYU
		}
